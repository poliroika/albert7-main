...
- Proof execution must name or inherit a typed execution environment. When a proof depends on a probed capability such as `desktop_gui_runtime`, its `proof.execution.execution_environment_id` must match the capability probe binding; an import-only Tkinter check is only `desktop_gui_headless`, not real desktop runtime.
