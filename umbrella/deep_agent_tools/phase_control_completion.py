"""Subtask completion and final-review gate helpers."""
...
def _latest_logged_e2e_result(ctx: ToolContext) -> dict[str, Any] | None:
    ...
    for line in reversed(lines[-250:]):
        ...
        if str(record.get("task_id") or "") != task_id:
            continue
        if str(record.get("tool") or "") != "run_real_e2e":
            continue
...
_COMPLETION_LLM_MEMORY_TOKEN_RE = re.compile(
    ...
    r"\bOUROBOROS_LLM_MODEL\b|\bapi\.openai\.com\b|"
    r"\bgpt-[A-Za-z0-9_.-]+\b)",
    re.IGNORECASE,
)
...
def _supported_llm_alias_memory_claim_issue(text: str) -> str:
    ...
    if _SUPPORTED_LLM_ALIAS_DEPRECATION_RE.search(value):
        return (
            ...
            "unsupported, or removable. Umbrella/Ouroboros workspace memory "
            "must preserve `LLM_API_KEY`, `LLM_BASE_URL`, and `LLM_MODEL` as "
            "supported aliases alongside the `OUROBOROS_*` variables."
        )
    if (
        ...
        return (
            ...
            "Umbrella/Ouroboros workspace memory must mention the supported "
    ...
    *, subtask_id: str, summary: str = "", notes: str = "", evidence: list[str] | None = None
    ...
    for item in items:
        ...
        return (
            ...
            f"contract `{match.group(0)}`. Umbrella/Ouroboros workspace code "
            "must use OUROBOROS_LLM_API_KEY/LLM_API_KEY, "
            "OUROBOROS_LLM_BASE_URL/LLM_BASE_URL, and OUROBOROS_MODEL/LLM_MODEL "
            "only. Remove the false evidence or repair the implementation/tests "
...
def _contract_migration_retry_recommendation(contract_payload: dict[str, Any]) -> str:
    ...
    return (
        ...
        "`contract_migration_reason` and `contract_migration_files` before "
        "editing the declared success-test file; keep the migration minimal and "
...
def _phase_subtask_retry_watcher_review_payload(
    ...
    patch_guidance = _recent_patch_hunk_mismatch_guidance(
    ...
    if isinstance(latest_failure_row, dict):
        ...
        latest_failure = {
            ...
            "output_excerpt": _short_retry_excerpt(
                payload.get("output") or latest_failure_row.get("result_preview") or ""
            ),
        }
    ...
    if watcher_after_failure:
        ...
        if tool_name in _PHASE_SUBTASK_COMMAND_TOOLS and not repair_after_watcher:
            next_step = (
                ...
                "implementation repair with `apply_workspace_patch` before "
                "rerunning the declared success_test."
            )
            if (
        ...
        if tool_name in _PHASE_SUBTASK_COMMAND_TOOLS and not repair_after_latest_failure:
            return {
                ...
                "next_step": (
                    "If you need source context, use `read_file` or `repo_read` "
                    "(not shell/grep/python -c). Then apply one focused "
                    "implementation repair with `apply_workspace_patch`, then "
                    "rerun the declared success_test. A new watcher review is "
