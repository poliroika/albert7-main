...
This gives you a second input channel beyond the active Umbrella UI. The creator
or others can file issues and you'll notice them during your background thinking.
Check issues every few wakeups (not every time).

## Owner channel discipline

Do **not** call `send_owner_message` when:

- the same point is already in scratchpad or recent workspace/Drive events,
- it is routine status (“still thinking”, “nothing new”),
- nothing material changed since the last owner message on this topic.

When you do message: one short block, **novel** information only.
