...
def ensure_claude_code_cli() -> bool:
    ...
    has_cli = (
        subprocess.run(
            ["bash", "-lc", "command -v claude >/dev/null 2>&1"], check=False
        ).returncode
        == 0
    )
    if has_cli:
    ...
    subprocess.run(
        ["bash", "-lc", "curl -fsSL https://claude.ai/install.sh | bash"], check=False
    )
    has_cli = (
        subprocess.run(
            ["bash", "-lc", "command -v claude >/dev/null 2>&1"], check=False
...
from ouroboros.llm import DEFAULT_LIGHT_MODEL, DEFAULT_LLM_BASE_URL
...
def get_cfg(
    name: str, default: str | None = None, allow_legacy_secret: bool = False
) -> str | None:
    ...
    if allow_legacy_secret:
        ...
        if legacy is not None and str(legacy).strip() != "":
            if name not in _LEGACY_CFG_WARNED:
                print(
                    f"[cfg] DEPRECATED: move {name} from Colab Secrets to config cell/env."
                )
...
def _parse_int_cfg(raw: str | None, default: int, minimum: int = 0) -> int:
    try:
        val = int(str(raw))
    except Exception:
        val = default
...
LLM_API_KEY = get_secret("OUROBOROS_LLM_API_KEY", default=OPENROUTER_API_KEY or "")
assert LLM_API_KEY and str(LLM_API_KEY).strip(), (
    "OUROBOROS_LLM_API_KEY or OPENROUTER_API_KEY not set. Add one to Colab Secrets (see README)."
)
...
assert GITHUB_USER and str(GITHUB_USER).strip(), (
    "GITHUB_USER not set. Add it to your config cell (see README)."
)
assert GITHUB_REPO and str(GITHUB_REPO).strip(), (
    "GITHUB_REPO not set. Add it to your config cell (see README)."
)
MAX_WORKERS = int(
    get_cfg("OUROBOROS_MAX_WORKERS", default="5", allow_legacy_secret=True) or "5"
)
MODEL_MAIN = get_cfg(
    "OUROBOROS_MODEL", default="anthropic/claude-sonnet-4.6", allow_legacy_secret=True
)
MODEL_CODE = get_cfg(
...
MODEL_LIGHT = get_cfg(
    "OUROBOROS_MODEL_LIGHT", default=DEFAULT_LIGHT_MODEL, allow_legacy_secret=True
)
LLM_BASE_URL = get_cfg(
    "OUROBOROS_LLM_BASE_URL", default=DEFAULT_LLM_BASE_URL, allow_legacy_secret=True
)
...
SOFT_TIMEOUT_SEC = max(
    60,
    int(
        get_cfg("OUROBOROS_SOFT_TIMEOUT_SEC", default="600", allow_legacy_secret=True)
        or "600"
    ),
)
HARD_TIMEOUT_SEC = max(
    120,
    int(
        get_cfg("OUROBOROS_HARD_TIMEOUT_SEC", default="1800", allow_legacy_secret=True)
        or "1800"
    ),
)
...
os.environ["OUROBOROS_LLM_BASE_URL"] = str(LLM_BASE_URL or DEFAULT_LLM_BASE_URL)
...
append_jsonl(
    DRIVE_ROOT / "logs" / "supervisor.jsonl",
    {
        ...
        "worker_start_method": str(
            os.environ.get("OUROBOROS_WORKER_START_METHOD") or ""
        ),
...
def _handle_supervisor_command(text: str, chat_id: int, tg_offset: int = 0):
    ...
    if lowered.startswith("/status"):
        status = status_text(
            WORKERS, PENDING, RUNNING, SOFT_TIMEOUT_SEC, HARD_TIMEOUT_SEC
        )
...
while True:
    ...
    for upd in updates:
        ...
        if agent._busy:
            ...
            while time.time() < _batch_deadline:
                ...
                if (
                    not _extra_updates
                    and (time.time() - _batch_start) < _EARLY_EXIT_SEC
                ):
                ...
                for _upd in _extra_updates:
                    ...
                    if (
                        _uid2
                        and _batch_state.get("owner_id")
                        and _uid2 == int(_batch_state["owner_id"])
                    ):
                        log_chat("in", _cid2, _uid2, _txt2)
                        _batch_state["last_owner_message_at"] = datetime.datetime.now(
                        ...
                        if _txt2.strip().lower().startswith("/"):
                            try:
                                _cmd_result = _handle_supervisor_command(
                                    _txt2, _cid2, tg_offset=offset
                                )
                                if _cmd_result is True:
                                    ...
                                    _txt2 = (
                                        _cmd_result + _txt2
                                    )  # dual-path: prepend note
                            except SystemExit:
                                raise
                            except Exception:
                                log.warning(
                                    "Supervisor command in batch failed", exc_info=True
                                )
                        if _txt2:
                            _batched_texts.append(_txt2)
                            _batch_deadline = max(
                                _batch_deadline, time.time() + 0.3
                            )  # extend for burst
            ...
            if len(_batched_texts) > 1:
                final_text = "\n\n".join(_batched_texts)
                log.info(
                    "Message batch: %d messages merged into one", len(_batched_texts)
                )
            elif _batched_texts:
    ...
    if DIAG_HEARTBEAT_SEC > 0 and (now_epoch - _last_diag_heartbeat_ts) >= float(
        DIAG_HEARTBEAT_SEC
    ):
