...
def test_phase_run_drive_state_omits_duplicate_task_input():
    ...
    phase_input = (
        "# Phase: execute\n"
        "## [ALWAYS-LOADED MEMORY]\n"
        "duplicate phase body from Umbrella\n"
    )
    ...
    task = {
        ...
        "id": "run-1:execute",
        "input": phase_input,
        "context_overlays": {"phase_prompt_rendered_by_umbrella": True},
    }
    ...
    def _safe_read(path, fallback=""):
        if str(path).replace("\\", "/").endswith("state/state.json"):
            return state_json
        return ""

    with (
        patch("ouroboros.context._read_prompt", return_value="SYSTEM"),
        patch("ouroboros.context._safe_read", side_effect=_safe_read),
        patch("ouroboros.context._build_memory_sections", return_value=[]),
        patch("ouroboros.context._build_recent_sections", return_value=[]),
        patch("ouroboros.context._build_runtime_section", return_value=""),
        patch("ouroboros.context._build_health_invariants", return_value=""),
    ...
    assert "duplicate phase body from Umbrella" not in system_text
    assert "task_input_omitted" in system_text
    assert sum(
        "duplicate phase body from Umbrella" in str(msg.get("content") or "")
        for msg in messages
    ) == 1
