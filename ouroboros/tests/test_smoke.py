...
TOOL_MODULES = [
    ...
    # "ouroboros.tools.review",  # requires httpx
]
...
EXPECTED_TOOLS = [
    ...
    "knowledge_write",
    "knowledge_list",
    # multi_model_review removed - requires httpx
    # GitHub Issues
    "list_github_issues",
    "get_github_issue",
    "comment_on_issue",
    ...
    "enable_tools",
    # Umbrella integration tools (from umbrella_tools.py)
    ...
    "bg_kill",
    "bg_list",
    # Misc additions surfaced by the registry
    "load_skill",
    "run_python_code",
    "web_fetch",
    "python_eval",
...
def test_memory_persistence():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert "test persistence content" in content, (
...
def test_no_hardcoded_replies():
    ...
    for root, dirs, files in os.walk(REPO / "ouroboros"):
        dirs[:] = [
            d
            for d in dirs
            if d not in ("__pycache__", ".venv", "venv", "site-packages")
        ]
        for f in files:
...
def test_no_env_dumping():
    ...
    for root, dirs, files in os.walk(REPO):
        dirs[:] = [
            ...
            for d in dirs
            if d
            not in (".git", "__pycache__", "tests", ".venv", "venv", "site-packages")
        ]
        for f in files:
            if not f.endswith(".py"):
                continue
