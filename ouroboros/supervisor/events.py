...
def _find_duplicate_task(desc: str, pending: list, running: dict) -> str | None:
...
def _handle_schedule_task(evt: dict[str, Any], ctx: Any) -> None:
    ...
    if depth > 3:
        log.warning(
            "Rejected task due to depth limit: depth=%d, desc=%s", depth, desc[:100]
        )
        if owner_chat_id:
            ctx.send_with_budget(
                int(owner_chat_id), "⚠️ Task rejected: subtask depth limit (3) exceeded"
            )
        return
    ...
    if owner_chat_id and desc:
        ...
        if dup_id:
            log.info(
                "Rejected duplicate task: new='%s' duplicates='%s'", desc[:100], dup_id
            )
            ctx.send_with_budget(
                int(owner_chat_id),
                f"⚠️ Task rejected: semantically similar to already active task {dup_id}",
            )
            return
...
def _handle_toggle_evolution(evt: dict[str, Any], ctx: Any) -> None:
    ...
    if st.get("owner_chat_id"):
        ...
        ctx.send_with_budget(
            int(st["owner_chat_id"]), f"🧬 Evolution: {state_str} (via agent tool)"
        )
