...
try:
    import requests
except ImportError:  # pragma: no cover - exercised indirectly in smoke environments
    requests = None
...
def _require_requests():
    """Return the requests module or raise a helpful runtime error."""
    if requests is None:
        raise RuntimeError(
            "Telegram support requires the 'requests' package. Install Ouroboros "
            "with `pip install -e .` or install `requests` explicitly."
        )
    return requests
...
class TelegramClient:
    ...
    def send_message(
        ...
        for attempt in range(3):
            try:
                ...
                r = http.post(f"{self.base}/sendMessage", data=payload, timeout=30)
                r.raise_for_status()
                data = r.json()
    ...
    def send_chat_action(self, chat_id: int, action: str = "typing") -> bool:
        ...
        try:
            ...
            log.debug(
                "Failed to send chat action to chat_id=%d", chat_id, exc_info=True
            )
            return False

    def send_photo(
        self, chat_id: int, photo_bytes: bytes, caption: str = ""
    ) -> tuple[bool, str]:
    ...
    def download_file_base64(
        self, file_id: str, max_bytes: int = 10_000_000
    ) -> tuple[str | None, str]:
        ...
        try:
            ...
            # Get file path
            r = http.get(
                f"{self.base}/getFile", params={"file_id": file_id}, timeout=10
            )
            r.raise_for_status()
            data = r.json()
            if not data.get("ok"):
            ...
            r2 = http.get(download_url, timeout=30)
            r2.raise_for_status()
            ...
            log.warning(
                "Failed to download file_id=%s from Telegram", file_id, exc_info=True
            )
            return None, ""
...
def _send_markdown_telegram(chat_id: int, text: str) -> tuple[bool, str]:
    ...
    for md_part in chunks:
        ...
        ok, err = tg.send_message(
            chat_id, _sanitize_telegram_text(html_text), parse_mode="HTML"
        )
        if not ok:
