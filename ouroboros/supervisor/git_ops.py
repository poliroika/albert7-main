...
def ensure_repo_present() -> None:
    ...
    subprocess.run(
        ["git", "config", "user.name", "Ouroboros"], cwd=str(REPO_DIR), check=True
    )
    subprocess.run(
        ["git", "config", "user.email", "ouroboros@users.noreply.github.com"],
        cwd=str(REPO_DIR),
        check=True,
    )
...
def _copy_untracked_for_rescue(
    dst_root: pathlib.Path, max_files: int = 200, max_total_bytes: int = 12_000_000
) -> dict[str, Any]:
    out: dict[str, Any] = {
        "copied_files": 0,
        "skipped_files": 0,
...
def _create_rescue_snapshot(
    branch: str, reason: str, repo_state: dict[str, Any]
) -> dict[str, Any]:
    ...
    if rc_status == 0:
        atomic_write_text(
            rescue_dir / "status.porcelain.txt",
            status_txt + ("\n" if status_txt else ""),
        )
    ...
    if rc_diff == 0:
        atomic_write_text(
            rescue_dir / "changes.diff", diff_txt + ("\n" if diff_txt else "")
        )
    else:
    ...
    unpushed_lines = [
        ln for ln in (repo_state.get("unpushed_lines") or []) if str(ln).strip()
    ]
    if unpushed_lines:
        atomic_write_text(
            rescue_dir / "unpushed_commits.txt", "\n".join(unpushed_lines) + "\n"
        )

    atomic_write_text(
        rescue_dir / "rescue_meta.json", json.dumps(info, ensure_ascii=False, indent=2)
    )
    return info
...
def checkout_and_reset(
    branch: str, reason: str = "unspecified", unsynced_policy: str = "ignore"
) -> tuple[bool, str]:
    ...
    if policy != "ignore":
        ...
        if dirty_lines or unpushed_lines:
            ...
            elif policy in {"rescue_and_block", "rescue_and_reset"} and rescue_info.get(
                "error"
            ):
            ...
            if policy in {"block", "rescue_and_block"}:
                msg = (
                    f"Reset blocked ({detail}) to protect local changes.{rescue_suffix}"
                )
                append_jsonl(
                    DRIVE_ROOT / "logs" / "supervisor.jsonl",
                    {
    ...
    subprocess.run(
        ["git", "reset", "--hard", f"origin/{branch}"], cwd=str(REPO_DIR), check=True
    )
