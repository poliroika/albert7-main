...
def json_load_file(path: pathlib.Path) -> dict[str, Any] | None:
    try:
        if not path.exists():
            return None
...
def acquire_file_lock(
    lock_path: pathlib.Path, timeout_sec: float = 4.0, stale_sec: float = 90.0
) -> int | None:
    ...
    while (time.time() - started) < timeout_sec:
        try:
            ...
            try:
                ...
                log.debug(
                    f"Failed to write lock metadata to {lock_path}", exc_info=True
                )
                pass
            return fd
            ...
            try:
                ...
                log.debug(
                    f"Failed to check/remove stale lock at {lock_path}", exc_info=True
                )
                pass
            time.sleep(0.05)
        except Exception:
...
def release_file_lock(lock_path: pathlib.Path, lock_fd: int | None) -> None:
    if lock_fd is None:
        return
    try:
        os.close(lock_fd)
...
def init_state() -> dict[str, Any]:
    ...
    try:
        ...
        if ground_truth is not None:
            ...
            st["openrouter_last_check_at"] = datetime.datetime.now(
...
def check_openrouter_ground_truth() -> dict[str, float] | None:
    """
    ...
    try:
        ...
        if not is_openrouter_base_url(resolve_llm_base_url()):
            return None
        api_key = (
            os.environ.get("OPENROUTER_API_KEY", "") or resolve_llm_api_key()
        ).strip()
        if not api_key:
            return None
...
def update_budget_from_usage(usage: dict[str, Any]) -> None:
    ...
    try:
        ...
        rounds = _to_int(
            usage.get("rounds") if isinstance(usage, dict) else 0, default=1
        )
        st["spent_calls"] = int(st.get("spent_calls") or 0) + rounds
        st["spent_tokens_prompt"] = _to_int(
            st.get("spent_tokens_prompt") or 0
        ) + _to_int(usage.get("prompt_tokens") if isinstance(usage, dict) else 0)
        st["spent_tokens_completion"] = _to_int(
            st.get("spent_tokens_completion") or 0
        ) + _to_int(usage.get("completion_tokens") if isinstance(usage, dict) else 0)
        st["spent_tokens_cached"] = _to_int(
            st.get("spent_tokens_cached") or 0
        ) + _to_int(usage.get("cached_tokens") if isinstance(usage, dict) else 0)
        ...
        if ground_truth is not None:
            ...
            try:
                ...
                if session_total_snap is not None and session_spent_snap is not None:
                    ...
                    if or_delta > 0.001:
                        drift_pct = (
                            abs(or_delta - our_delta) / max(abs(or_delta), 0.01) * 100.0
                        )
...
def status_text(
    ...
    lines.append(
        f"version: {st.get('current_branch')}@{(st.get('current_sha') or '')[:8]}"
    )
    busy_count = sum(
        1 for w in workers_dict.values() if getattr(w, "busy_task_id", None) is not None
    )
    ...
    if running_dict:
        ...
        for task_id, meta in list(running_dict.items())[:10]:
            ...
            started = (
                float(meta.get("started_at") or 0.0) if isinstance(meta, dict) else 0.0
            )
            hb = (
    ...
    if breakdown:
        ...
        breakdown_parts = [
            f"{cat}=${cost:.2f}" for cat, cost in sorted_categories if cost > 0
        ]
        if breakdown_parts:
    ...
    if drift_pct is not None:
        ...
        if (
            ...
            and session_spent_snap is not None
    ...
    if models:
        ...
        for model_name, stats in sorted_models:
            if stats["cost"] > 0 or stats["calls"] > 0:
                ...
                lines.append(
                    f"  {model_name}: ${cost:.2f} ({calls} calls, {pt:,}p/{ct:,}c tok)"
                )
...
def rotate_chat_log_if_needed(
    drive_root: pathlib.Path, max_bytes: int = 800_000
) -> None:
