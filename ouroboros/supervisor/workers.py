...
_LAST_SPAWN_TIME: float = (
    0.0  # grace period: don't count dead workers right after spawn
)
_SPAWN_GRACE_SEC: float = (
    90.0  # workers need up to ~60s to init on Colab (spawn + pip + Drive FUSE)
)
...
_WORKER_START_METHOD = (
    str(
        os.environ.get("OUROBOROS_WORKER_START_METHOD", _DEFAULT_WORKER_START_METHOD)
        or _DEFAULT_WORKER_START_METHOD
    )
    .strip()
    .lower()
)
...
def handle_chat_direct(
    chat_id: int,
    text: str,
    image_data: tuple[str, str] | tuple[str, str, str] | None = None,
) -> None:
    try:
...
def auto_resume_after_restart() -> None:
    ...
    try:
        ...
        if not stripped or stripped == "# Scratchpad" or "(empty" in stripped.lower():
            ...
            content_lines = [
                ln for ln in content_lines if not ln.startswith("UpdatedAt:")
            ]
...
def worker_main(
    wid: int, in_q: Any, out_q: Any, repo_dir: str, drive_root: str
) -> None:
    import sys as _sys
...
def _log_worker_crash(
    wid: int, drive_root: pathlib.Path, phase: str, exc: Exception, tb: str
) -> None:
...
def _first_worker_boot_event_since(offset_bytes: int) -> dict[str, Any] | None:
...
def _verify_worker_sha_after_spawn(
    events_offset: int, timeout_sec: float = 90.0
) -> None:
...
def spawn_workers(n: int = 0) -> None:
    ...
    for i in range(count):
        in_q = _CTX.Queue()
        proc = _CTX.Process(
            target=worker_main, args=(i, in_q, _EVENT_Q, str(REPO_DIR), str(DRIVE_ROOT))
        )
        proc.daemon = True
        proc.start()
    ...
    threading.Thread(
        target=_verify_worker_sha_after_spawn, args=(events_offset,), daemon=True
    ).start()
        ...
        for w in WORKERS.values():
            if w.busy_task_id is None and PENDING:
                ...
                if chosen_idx is None:
                    ...
                    PENDING[:] = [
                        t for t in PENDING if str(t.get("type") or "") != "evolution"
                    ]
