...
def compute_complexity_metrics(sections: list[tuple[str, str]]) -> dict[str, Any]:
    ...
    oversized_functions = [
        (p, start, length) for p, start, length in function_lengths if length > 150
    ]
    oversized_modules = [
        (p, lines) for p, lines in file_sizes if p.endswith(".py") and lines > 1000
    ]
            ...
            for fn in sorted(filenames):
                try:
                    ...
                    if (total_chars + len(content)) > max_total_chars:
                        content = clip_text(
                            content, max(2000, max_total_chars - total_chars)
                        )
                        truncated += 1
...
def chunk_sections(
    sections: list[tuple[str, str]], chunk_token_cap: int = 70_000
) -> list[str]:
