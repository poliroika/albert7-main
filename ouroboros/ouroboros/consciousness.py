...
class BackgroundConsciousness:
    ...
    def __init__(
        ...
        event_queue: Any,
        owner_chat_id_fn: Callable[[], int | None],
    ):
    ...
    def _check_budget(self) -> bool:
        ...
        try:
            ...
            log.warning(
                "Failed to check background consciousness budget", exc_info=True
            )
            return True
        ...
        try:
            for round_idx in range(1, self._MAX_BG_ROUNDS + 1):
                ...
                try:
                    ...
                    log.debug(
                        "Failed to update global budget from BG consciousness",
                        exc_info=True,
                    )
    ...
    def _execute_tool(
        self, tc: dict[str, Any], all_pending_events: list[dict[str, Any]]
    ) -> str:
        ...
        append_jsonl(
            self._drive_root / "logs" / "tools.jsonl",
            {
                ...
                "result_preview": sanitize_tool_result_for_log(
                    truncate_for_log(result_str, 2000)
                ),
            },
        )
