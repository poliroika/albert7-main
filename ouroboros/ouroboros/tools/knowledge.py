...
def _sanitize_topic(topic: str) -> str:
    ...
    if not _VALID_TOPIC.match(topic):
        raise ValueError(
            f"Invalid topic name: {topic}. Use alphanumeric, underscore, hyphen, dot."
        )
...
def _rebuild_index(ctx: ToolContext):
    ...
    for f in sorted(kdir.glob("*.md")):
        ...
        try:
            ...
            log.debug(
                f"Failed to read knowledge file for index rebuild: {topic}",
                exc_info=True,
            )
...
def _update_index_entry(ctx: ToolContext, topic: str):
    ...
    if topic_path.exists():
        try:
            ...
            log.debug(
                f"Failed to read knowledge file for index update: {topic}",
                exc_info=True,
            )
