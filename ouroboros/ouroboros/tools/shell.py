...
def _run_shell(ctx: ToolContext, cmd, cwd: str = "", timeout_seconds: int = 120) -> str:
    ...
    try:
        ...
        return f"⚠️ TIMEOUT: command exceeded {timeout_seconds}s."
        ...
        if status_res.returncode == 0 and status_res.stdout.strip():
            ...
            if diff_res.returncode == 0 and diff_res.stdout.strip():
                return (
                    ...
                    f"Remember to run git_status and repo_commit_push for a local commit."
                )
    except Exception as e:
        log.debug(
            "Failed to check git status after claude_code_edit: %s", e, exc_info=True
        )
    return ""
...
def get_tools() -> list[ToolEntry]:
    return [
        ToolEntry(
            ...
            {
                "name": "run_shell",
                "description": "Run a shell command (list of args) inside the Ouroboros repo. For host workspaces use run_workspace_command.",
                "parameters": {
                    "type": "object",
                    "properties": {
        ...
        ToolEntry(
            "claude_code_edit",
            {
                ...
                "description": "Delegate code edits to Claude Code CLI. Preferred for multi-file changes and refactors. Follow with repo_commit_push for a local commit.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "prompt": {"type": "string"},
