...
def _reset_playwright_greenlet():
    ...
    for k in mods_to_remove:
        try:
            ...
            log.debug(
                f"Failed to delete greenlet module {k} during reset", exc_info=True
            )
            pass
...
def _ensure_browser(ctx: ToolContext):
    ...
    if _pw_instance is not None and _pw_thread_id != current_thread_id:
        log.info(
            f"Thread switch detected (old={_pw_thread_id}, new={current_thread_id}). Resetting Playwright..."
        )
    ...
    if ctx.browser_state.browser is not None:
        try:
            ...
            log.debug(
                "Browser connection check failed in _ensure_browser", exc_info=True
            )
            pass
    ...
    if _pw_instance is None:
        ...
        try:
            ...
            if "cannot switch" in str(e) or "different thread" in str(e):
                ...
                log.info(
                    f"Recreated Playwright instance in thread {_pw_thread_id} after error"
                )
            else:
                raise
...
def _browse_page(
    ...
    try:
        ...
        if (
            "cannot switch" in str(e)
            or "different thread" in str(e)
            or "greenlet" in str(e).lower()
        ):
            log.warning(
                f"Browser thread error detected: {e}. Resetting Playwright and retrying..."
            )
            cleanup_browser(ctx)
            _reset_playwright_greenlet()
            page = _ensure_browser(ctx)
