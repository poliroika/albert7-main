                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "models": {
                            ...
                            "items": {"type": "string"},
                            "description": (
                                "Model identifiers for the configured LLM API "
...
def _handle_multi_model_review(
    ctx: ToolContext, content: str = "", prompt: str = "", models: list = None
) -> str:
    ...
    try:
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
                result = pool.submit(
                    asyncio.run, _multi_model_review_async(content, prompt, models, ctx)
                ).result()
        except RuntimeError:
...
async def _query_model(client, url, model, messages, api_key, semaphore):
...
async def _multi_model_review_async(
    content: str, prompt: str, models: list, ctx: ToolContext
):
    ...
    if not models:
        return {
            "error": "models list is required (e.g. ['openai/o3', 'google/gemini-2.5-pro'])"
        }
    ...
    if len(models) > MAX_MODELS:
        return {
            "error": f"Too many models requested ({len(models)}). Maximum is {MAX_MODELS}."
        }
    ...
    api_url = chat_completions_url(resolve_llm_base_url())
    ...
    async with httpx.AsyncClient() as client:
        tasks = [
            _query_model(client, api_url, m, messages, api_key, semaphore)
            for m in models
        ]
