...
from ouroboros.llm import use_anthropic_style_cache_extensions
...
def _build_memory_sections(memory: Memory) -> list[str]:
    """Build workspace-scoped scratchpad sections."""
    sections = []
...
def _build_recent_sections(memory: Memory, env: Any, task_id: str = "") -> list[str]:
    ...
    # Recent terminal scrollback: the persistent shell appends every command
    # and its output to <drive_root>/memory/terminal_scrollback.md so the
    # model can re-read prior terminal state even after _maybe_compact_history
    # drops the raw tool messages. Tail it here as a context section so the
    # information shows up automatically every round.
    try:
        ...
        if scrollback_path.exists():
            ...
            if scrollback_raw.strip():
                ...
                tail_text = "\n".join(tail_lines)
                sections.append("## Recent terminal\n\n" + clip_text(tail_text, 12000))
    except Exception:
        log.debug("Failed to build Recent terminal section", exc_info=True)
...
def _build_health_invariants(env: Any) -> str:
    ...
    try:
        ...
        if ver_file and pyproject_ver and ver_file != pyproject_ver:
            checks.append(
                f"CRITICAL: VERSION DESYNC — VERSION={ver_file}, pyproject.toml={pyproject_ver}"
            )
        elif ver_file:
    ...
    try:
        ...
        def _scan_file_for_injected(
            path, type_field="type", type_value="owner_message_injected"
        ):
            if not path.exists():
                return
...
def _workspace_verification_digest_block(env: Any, workspace_id: str) -> str:
    """Inject resolved verification steps when Umbrella is available (optional dependency)."""

    wid = (workspace_id or "").strip()
    if not wid:
        return ""
    candidates: list[pathlib.Path] = []
    host = getattr(env, "host_repo_root", None)
    if host:
        candidates.append(pathlib.Path(host).resolve() / "workspaces" / wid)
    repo_dir = getattr(env, "repo_dir", None)
    if repo_dir:
        rp = pathlib.Path(repo_dir).resolve()
        candidates.append(rp / "workspaces" / wid)
        candidates.append(rp.parent / "workspaces" / wid)
    ...
    try:
        ...
        return format_workspace_verification_digest(ws)
    except Exception:
        log.debug("workspace verification digest unavailable", exc_info=True)
        return ""


_WORKSPACE_PROMPT_FILES = {
    "SYSTEM": ("prompts/SYSTEM.md", "SYSTEM.md"),
    "BIBLE": ("BIBLE.md", "BIBLE.md"),
    "CONSCIOUSNESS": ("prompts/CONSCIOUSNESS.md", "CONSCIOUSNESS.md"),
}


def _current_workspace_id(env: Any) -> str:
    try:
        state = json.loads(
...
def _workspace_prompt_path(
    env: Any, workspace_id: str, name: str
) -> pathlib.Path | None:
    host_root = getattr(env, "host_repo_root", None)
    if not host_root or not workspace_id:
        return None
    clean = workspace_id.replace("\\", "/").strip("/")
    if not clean or ".." in pathlib.Path(clean).parts:
        return None
    return (
        pathlib.Path(host_root).resolve()
        / "workspaces"
        / clean
        / ".memory"
        / "prompts"
...
def _read_prompt(env: Any, workspace_id: str, name: str, fallback: str = "") -> str:
    repo_rel, _overlay_name = _WORKSPACE_PROMPT_FILES[name]
    overlay = _workspace_prompt_path(env, workspace_id, name)
    if overlay is not None:
        text = _safe_read(overlay)
        if text.strip():
            return text
    return _safe_read(env.repo_path(repo_rel), fallback=fallback)
...
def build_llm_messages(
    ...
    memory: Memory,
    task: dict[str, Any],
    review_context_builder: Any | None = None,
    ...
    Args:
        ...
        memory: Memory instance for scratchpad/logs
    ...
    workspace_id = _current_workspace_id(env)
    ...
    # Block 2: Semi-stable content (scratchpad + knowledge) — cached
    ...
    if consciousness_md.strip():
        static_text += "\n\n## CONSCIOUSNESS.md\n\n" + clip_text(
            consciousness_md, 60000
        )
    if needs_full_context:
        static_text += "\n\n## README.md\n\n" + clip_text(readme_md, 180000)

    # Semi-stable content: workspace scratchpad and knowledge
    ...
    dynamic_parts.extend(
        _build_recent_sections(memory, env, task_id=task.get("id", ""))
    )
    ...
    digest = _workspace_verification_digest_block(env, workspace_id)
    if digest:
        dynamic_parts.append(digest)
    ...
    # OpenRouter+Anthropic: multipart system + cache_control (prompt caching).
    # Plain OpenAI-compatible gateways (many internal /v1 proxies) reject unknown keys → HTTP 400.
    if use_anthropic_style_cache_extensions():
        ...
        system_body = static_text + "\n\n" + semi_stable_text + "\n\n" + dynamic_text
        system_message = {"role": "system", "content": system_body}
    ...
    for i, msg in enumerate(messages):
        ...
        if parent_round is not None and parent_round in rounds_to_compact:
            ...
            if len(content) > 120:
                ...
                old_results.append(
                    {"idx": i, "tool_call_id": tool_call_id, "content": content[:1500]}
                )
...
def _task_context_overlay(task: dict[str, Any], name: str) -> str:
