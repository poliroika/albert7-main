...
Scratchpad, chat history, and workspace-scoped logs.
"""
...
class Memory:
    """Ouroboros memory management: workspace scratchpad, chat history, logs."""

    def __init__(
        self, drive_root: pathlib.Path, repo_dir: pathlib.Path | None = None
    ):
        ...
        try:
            ...
            for line in raw_lines:
                ...
                try:
                    ...
                    log.debug(
                        f"Failed to parse JSON line in chat_history: {line[:100]}"
                    )
                    continue
            ...
            if search:
                ...
                entries = [
                    e for e in entries if search_lower in str(e.get("text", "")).lower()
                ]
    ...
    def read_jsonl_tail(
        self, log_name: str, max_entries: int = 100
    ) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for line in tail:
                ...
                try:
                    ...
                    log.debug(
                        f"Failed to parse JSON line in read_jsonl_tail: {line[:100]}",
                        exc_info=True,
                    )
                    continue
            return entries
    ...
    def summarize_tools(self, entries: list[dict[str, Any]]) -> str:
        ...
        for e in entries[-10:]:
            ...
            status = (
                ...
                if (
                    "result_preview" in e
                    and not str(e.get("result_preview", "")).lstrip().startswith("⚠️")
                )
                else "·"
            )
    ...
    def summarize_events(self, entries: list[dict[str, Any]]) -> str:
        ...
        if errors:
            ...
            for e in errors[-10:]:
                lines.append(
                    f"  {e.get('type', '?')}: {short(str(e.get('error', '')), 120)}"
                )
        return "\n".join(lines)
