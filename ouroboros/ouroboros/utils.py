...
def append_jsonl(path: pathlib.Path, obj: dict[str, Any]) -> None:
    ...
    try:
        ...
        log.warning(
            "append_jsonl: all write attempts failed for %s", path, exc_info=True
        )
    finally:
        ...
        if lock_acquired:
            try:
                ...
                log.debug(
                    "Failed to unlink lock file after jsonl append", exc_info=True
                )
                pass
    ...
    try:
        ...
        if text_len > threshold:
            ...
            try:
                task_id = task.get("id")
                filename = (
                    f"task_{task_id}.txt" if task_id else f"task_{text_hash[:12]}.txt"
                )
                ...
                log.debug(
                    "Failed to persist full task text to Drive during sanitization",
                    exc_info=True,
                )
                pass
        else:
...
def sanitize_tool_args_for_log(
    ...
    def _sanitize_value(key: str, value: Any, depth: int) -> Any:
        ...
        try:
            ...
            log.debug(
                "Failed to JSON serialize value in sanitize_tool_args", exc_info=True
            )
            return {"_repr": repr(value)}
