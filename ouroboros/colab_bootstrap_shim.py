...
def export_secret_to_env(name: str, required: bool = False) -> str | None:
...
_primary_llm_key = export_secret_to_env("OUROBOROS_LLM_API_KEY", required=False)
if not _primary_llm_key:
    _primary_llm_key = export_secret_to_env("OPENROUTER_API_KEY", required=False)
if _primary_llm_key:
    os.environ["OUROBOROS_LLM_API_KEY"] = str(_primary_llm_key)
assert _primary_llm_key is not None and str(_primary_llm_key).strip() != "", (
    "Missing required secret: OUROBOROS_LLM_API_KEY or OPENROUTER_API_KEY"
)
...
if _rc == 0:
    ...
    subprocess.run(
        ["git", "push", "-u", "origin", BOOT_BRANCH], cwd=str(REPO_DIR), check=True
    )
    _STABLE = f"{BOOT_BRANCH}-stable"
    subprocess.run(["git", "branch", _STABLE], cwd=str(REPO_DIR), check=True)
    subprocess.run(
        ["git", "push", "-u", "origin", _STABLE], cwd=str(REPO_DIR), check=True
    )
